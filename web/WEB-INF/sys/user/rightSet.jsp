...
<s:form action="user_saveQueryRight.do" id="userForm" name="userForm" onsubmit="return CheckForm(this);" method="post">
...
<fieldset>
    <legend>系统用户(<s:property value="name"/>)设置能查询的外呼员</legend>
    <div class="Single">
      <table class="infoList" cellspacing="1" cellpadding="0" rules="all" border="0" id="grid1" style="border-width:0px;width:100%;">
	    ...
	    <s:iterator id="var" value="userList" status="vStatus">
	    <tr class="<s:if test="#vStatus.count%2==1">trBg01</s:if><s:else>trBg02</s:else>" onclick="RowMouseClick(this,&quot;grid1_<s:property value="#vStatus.count"/>_CheckBoxButton&quot;);">
	      <td class="show" align="center">
	      	...
	      	<s:set name="sel_flag" value="'0'"></s:set>
	        <s:iterator id="var2" value="selUserList">
	          <s:if test="#var.id==#var2.user_id"><s:set name="sel_flag" value="'1'"/></s:if>
	        </s:iterator>
	      	<input id="grid1_<s:property value="#vStatus.count"/>_CheckBoxButton" type="checkbox" name="ids" value="<s:property value="#var.id"/>" <s:if test="#sel_flag=='1'">checked</s:if>/>
	      	</span>
	      </td>
