...
function deleteInfo() {
    ...
    $.messager.confirm('确定操作', '你确定要删除该条数据?', function(r){
		if (r){
			location = "dataLog_delete.do?id="+v[0]+"&pageNo=<s:property value="pageNo"/>";
		}
	});
}
...
function turnTo(){
	...
	if(flag){
		window.location="dataLog_list.do?redirectSign=0&pageNo="+pageNo+"&sTime=<s:property value="sTime" />&eTime=<s:property value="eTime"/>";
	}
}
...
function clearLog(){
   $.messager.confirm('确定操作', '你确定要清空日志信息？', function(r){
      if(r){
   		 window.location="dataLog_clear.do";
   	  }
   });	
}
function viewInfo(){
	var v = GetGridSelect('grid1');
 	if (v.length == 0 || v[0] == "0") {
 		$.messager.alert('未选中记录','请先选中需要查看的日志信息。');
        return;
    }
	showSuperDialog({ url: 'dataLog_view.do?id=' +v[0], title: '浏览日志信息', width: 715, height:340, model: false });
}
</script>
</head>
<body>
<br/>
<s:form name="form1" id="form1" action="dataLog_list.do" method="post">
<div class="searchPanel" style="margin-left:12px;">
    ...
    <div class="searchPanel-body">
       ...
       <s:textfield id="t_etime" name="eTime" cssClass="easyui-datebox OnBlurCSS" cssStyle="width:80px;" onblur="this.className='OnBlurCSS'" onfocus="this.className='OnFocusCSS'" ></s:textfield>
        <label>日志类型:</label>
        <s:select name="type" list="#{1:'正常',0:'异常'}" headerKey="" headerValue="--所有--"/> 
	...
	<thead>
	  <tr>
        	<td colspan="10">
	        	...
	        	<a href="#" class="easyui-linkbutton" plain="true" iconCls="icon-search" onclick="viewInfo()">查看日志</a> 
	...
	<table class="infoList" cellspacing="1" cellpadding="0" rules="all" border="0" id="grid1" style="width:98%;" align="center">
	  ...
	  <tfoot>
       <tr align="right">
		...
		<s:else>
        <a href="javascript:goPage(form1,'dataLog_list.do?redirectSign=0&pageNo=1');">首页</a> | 
        </s:else>
		<s:if test="pageNo==1">
		<a>上一页</a>|
		</s:if>
		<s:else>
        <a href="javascript:goPage(form1,'dataLog_list.do?redirectSign=0&pageNo=<s:property value="prevPageNo"/>');">上一页</a> | 
        </s:else>
		<s:if test="pageNo==totalPages">
		<a>下一页</a>|
		</s:if>
		<s:else>
        <a href="javascript:goPage(form1,'dataLog_list.do?redirectSign=0&pageNo=<s:property value="nextPageNo"/>');">下一页</a> | 
        </s:else>
		<s:if test="pageNo==totalPages">
		<a>尾页</a>&nbsp;
		</s:if>
		<s:else>
        <a href="javascript:goPage(form1,'dataLog_list.do?redirectSign=0&pageNo=<s:property value="totalPages"/>')">尾页</a> &nbsp;
        </s:else>
