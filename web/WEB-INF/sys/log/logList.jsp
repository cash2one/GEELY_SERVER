...
function deleteInfo() {
    ...
    $.messager.confirm('确定操作', '你确定要删除该条数据?', function(r){
		if (r){
			location = "sysLog_delete.do?logId="+v[0]+"&pageNo=<s:property value="pageNo"/>";
		}
	});
}
	...
	<thead>
	  ...
	  <tr class="<s:if test="#vStatus.count%2==1">trBg01</s:if><s:else>trBg02</s:else>" onclick="RowMouseClick(this,&quot;grid1_<s:property value="#vStatus.count"/>_CheckBoxButton&quot;);">
	    <td class="show" align="center">
	      <span KeyField="<s:property value="#var.logId"/>">
	    ...
	    <td align="center"><s:property value="#var.user.name"/></td>
	    <td align="center"><s:property value="#var.ipAddress"/></td>
	    <td align="center"><s:property value="#var.logContent"/></td>
	    <td align="center"><s:property value="#var.logTime"/></td>
	  </tr>
	  </s:iterator>
	  </tbody>
	  <tfoot>
