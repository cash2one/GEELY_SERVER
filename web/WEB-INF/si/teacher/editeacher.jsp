...
function CheckForm(form) {
	...
	if (!$("#spart").val()){
		$.messager.alert('信息输入未完整','请输入所在学段!!!');
   		$("#t_name").focus();
   		return false;
	}
	if (!$("#mobile").val()){
	...
	if (!$("#workno").val()){
		$.messager.alert('信息输入未完整','请输入教师工号!!!');
   		$("#t_name").focus();
   		return false;
	}
	if (!$("#loginname").val()){
	...
	if (!$("#score").val()){
		$.messager.alert('信息输入未完整','请输入教师积分!!!');
   		$("#t_name").focus();
   		return false;
	}
...
<s:form action="teacher_saveteacher.do" id="versionForm" name="versionForm" onsubmit="return CheckForm(this);" method="post" theme="simple" enctype="multipart/form-data">
<!--Content start -->
...
<fieldset class="VForm">
    ...
    <div class="Single">
     ...
     <div class="Single">
      <label>教师姓名：</label><s:textfield id="name" name="name" cssStyle="width:200px;" maxlength="30" valid="required|limit" errmsg="请输入产品名称|用户名最大长度为64" min="1" max="64"></s:textfield>
    ...
    <div class="Single">
      <label>教师工号：</label><s:textfield id="workno" name="workno" cssStyle="width:200px;" maxlength="30" valid="required|limit" errmsg="请输入产品名称|用户名最大长度为64" min="1" max="64"></s:textfield>
      <span style="color:Red">*
      <s:fielderror>
      <s:param>workno</s:param>
      </s:fielderror>
      </span>
    </div>
     <div class="Single">
      <label>所在学段：</label><s:select id="spart" name="spart" cssStyle="width:200px;" list="#{'01':'幼儿园','02':'小学','03':'初中','04':'高中','05':'其它'}"></s:select>
      <span style="color:Red">*
      <s:fielderror>
      <s:param>spart</s:param>  
      </s:fielderror>
      </span>
    </div>
    <div class="Single">
      <label>教师角色：</label><s:select id="role" name="role" cssStyle="width:200px;" list="#{'01':'班主任','02':'任课老师','03':'学校管理员'}"></s:select>
      <span style="color:Red">*
      <s:fielderror>
      <s:param>spart</s:param>  
      </s:fielderror>
    ...
    <div class="Single">
      <label>教师积分：</label><s:textfield id="score" name="score" cssStyle="width:200px;" maxlength="30" valid="required|limit" errmsg="请输入产品名称|用户名最大长度为64" min="1" max="64"></s:textfield>
      <span style="color:Red">*
      <s:fielderror>
